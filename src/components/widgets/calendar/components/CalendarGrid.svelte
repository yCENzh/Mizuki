<script lang="ts">
	...
	function getCellClass(cell: CalendarGridCell): string {
		...
		if (cell.isSelected) {
			bgClass = "bg-[var(--primary)] text-white shadow-md border border-transparent";
		} else if (cell.isToday) {
			bgClass = "text-[var(--primary)] font-bold bg-[var(--primary)]/10 border border-[var(--primary)]";
		} else if (cell.hasPost) {
			bgClass = "font-bold text-neutral-900 dark:text-neutral-100 hover:bg-[var(--btn-plain-bg-hover)] border border-transparent";
		}
